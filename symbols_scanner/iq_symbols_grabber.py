...
- บันทึกรายงานฉบับสมบูรณ์ไปที่: symbols_scanner/iq_symbols_MMDDHHMM.txt
"""
...
def main():
    ...
    ts_str = now_th.strftime("%m%d%H%M")
    output_path = current_dir / f"iq_symbols_{ts_str}.txt"

    print(f"[PATH] Settings Path : {settings_path.name}")
    print(f"[PATH] Output Report : {output_path.name} (iq_symbols_MMDDHHMM.txt)")
    ...
    try:
        ...
        if not email or not password:
            raise ValueError(f"iq_email or iq_password is missing in {settings_path.name}")
    ...
    for aid, ainfo in turbo_actives.items():
        sym_name = ainfo.get('name', '').replace('front.', '').strip()
        if sym_name and sym_name not in all_symbols:
    ...
    for sym in all_symbols:
        ...
        if turbo_open and binary_open:
            if turbo_pct >= binary_pct and turbo_pct > 0:
                best_route = f"TURBO ({turbo_pct}%)"
            elif binary_pct > 0:
                best_route = f"BINARY ({binary_pct}%)"
            else:
                best_route = "ปิดรับคำสั่ง"
        elif turbo_open and turbo_pct > 0:
            best_route = f"TURBO ({turbo_pct}%)"
    ...
    # ไม่ต้องมีระบบจัดเรียง (No sorting / no ranking) ตามคำสั่งบอส - บันทึกและแสดงตามลำดับดิบที่ดึงมาจาก IQ Option โดยตรง

    tz_thailand = timezone(timedelta(hours=7))
    now_dt = datetime.now(tz_thailand)
    now_str = now_dt.strftime("%Y-%m-%d %H:%M:%S")
    ts_str = now_dt.strftime("%m%d%H%M")
    output_path = current_dir / f"iq_symbols_{ts_str}.txt"
    ...
    # 7. Write to iq_symbols_{ts_str}.txt (Single File Lifecycle)
    for pattern in ["iq_symbols_*.txt", "all_symbols_*.txt", "active_symbols*.txt"]:
        for old_file in current_dir.glob(pattern):
            try:
                if old_file.is_file():
                    old_file.unlink()
            except Exception as e:
                print(f"[WARN] Failed to delete old report file {old_file.name}: {e}")
