...
class ModelBundle:
    ...
    def predict_proba(self, X: np.ndarray) -> np.ndarray:
        """Type-safe prediction method returning (N, 3) probabilities: [0: WAIT, 1: CALL, 2: PUT]."""
        ...
        if hasattr(self.model, "predict_proba"):
            ...
            if raw_probs.shape[1] == 2:
                p_put = raw_probs[:, 0]
                p_call = raw_probs[:, 1]
                p_wait = 1.0 - np.abs(p_call - p_put)
                probs_3 = np.zeros((len(X), 3), dtype=np.float64)
                probs_3[:, 0] = p_wait
                probs_3[:, 1] = p_call
                probs_3[:, 2] = p_put
                return probs_3
                ...
                padded = np.zeros((len(X), 3), dtype=np.float64)
                padded[:, 0] = 1.0
                return padded

        raise AttributeError("FAIL-FAST: Underlying model does not implement predict_proba")
    ...
    def evaluate(
        ...
        **kwargs
        ...
        try:
            ...
            if p_call >= (self.min_confidence / 100.0) and p_call > p_put:
                action = "CALL"
                conf = int(p_call * 100)
                reason = f"LightGBM: สัญญาณ CALL มั่นใจ {conf}% (Rejection ล่าง + สอดคล้องแนวโน้ม) ค่ะ"
            elif p_put >= (self.min_confidence / 100.0) and p_put > p_call:
                action = "PUT"
                conf = int(p_put * 100)
