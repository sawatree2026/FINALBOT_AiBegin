...
try:
    ...
    from .execution_gate.broker_executor import BrokerExecutor
    from .execution_gate.order_tracker import OrderTracker
    from .payload_sanitizer import payload_to_text, sanitize_payload, sanitize_payload_text
except ImportError:
    from data_trade.strategies_mode.execution_gate.gate_controller import ExecutionGate
    from data_trade.strategies_mode.execution_gate.money_manager import MoneyManager
    from data_trade.strategies_mode.execution_gate.broker_executor import BrokerExecutor
    from data_trade.strategies_mode.execution_gate.order_tracker import OrderTracker
    from data_trade.strategies_mode.payload_sanitizer import payload_to_text, sanitize_payload, sanitize_payload_text
        ...
        if should_execute_order:
            ...
            try:
                ...
                if order_data and order_data.get("status") == "SUCCESS":
                    ...
                    logger.warning(
                        f"[ExecutorManager] Broker order rejected for {symbol}: "
                        f"{order_data.get('error') if order_data else 'Unknown'}"
                    )
            except Exception as e:
                logger.exception(
                    f"[ExecutorManager] Broker execution failed for {symbol}: {e}"
                )
                order_data = {"status": "ERROR", "symbol": symbol, "action": action, "error": str(e)}
