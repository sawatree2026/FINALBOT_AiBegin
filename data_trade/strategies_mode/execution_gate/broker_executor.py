...
class BrokerExecutor:
    ...
    def execute_order(
        ...
        # ── Step 1: Protocol 1 (Binary / Turbo Standard Route) with Auto -op Fallback ─────
        start_t = time.time()
        last_error = None
        max_retries = 2
        
        candidate_symbols = []
        if not symbol.endswith("-op") and not symbol.endswith("-OTC"):
            candidate_symbols.append(f"{symbol}-op")
        candidate_symbols.append(str(symbol))

        for current_sym in candidate_symbols:
                ...
                try:
                    ...
                    if status and result_id is not None:
                        ...
                        logger.info(
                            ...
                            f"Symbol: {current_sym}, Action: {norm_action}, Elapsed: {elapsed}s, Attempt: {attempt}"
                        )
                        return {
                            "status": "SUCCESS",
                        ...
                        logger.warning(
                            ...
                            f"Symbol: {current_sym}, Action: {norm_action}, Reason: {last_error}"
                        )
                        if "not available" in str(last_error).lower() and current_sym != candidate_symbols[-1]:
                            break # Try next candidate symbol (-op) immediately
                except Exception as e:
                    last_error = str(e)
                    logger.exception(
                        f"[BrokerExecutor] Exception during order execution for {current_sym}: {e}"
                    )
                
                if attempt < max_retries:
                    time.sleep(0.5)

        # Strict Binary Option enforcement: Digital fallback disabled per boss mandate

        # ถ้าทั้ง Binary/Turbo และ Digital ปฏิเสธ ให้คืนค่า REJECTED อย่างปลอดภัย (ไม่ crash บอท)
        elapsed = round(time.time() - start_t, 3)
        error_reason = last_error or "Unknown broker rejection after retries"
        logger.error(
            f"[BrokerExecutor] Order REJECTED for {symbol} after retries: {error_reason}. "
            f"Preserving system execution loop."
        )
